...
function sort_function(application_1, application_2) {
    /** Compare gate score */
    if(Number(application_1['gate_score']) > Number(application_2['gate_score'])) return -1;
    if(Number(application_1['gate_score']) < Number(application_2['gate_score'])) return 1;

    /** Compare degree marks */
    var perccentage_1 = 0, percentage_2 = 0;
    ...
    if(degree_1_data_type === 'CGPA') {
        if(degree_1_cgpa_scale !== "") {
            percentage_1 = Number(degree_1_value) / Number(degree_1_cgpa_scale) * 100;
        }
        else {
    ...
    if(degree_2_data_type === 'CGPA') {
        if(degree_2_cgpa_scale !== "") {
            percentage_2 = Number(degree_2_value) / Number(degree_2_cgpa_scale) * 100;
        }
        else {
...
function get_merit_positions(data) {
    ...
    let current_position = 1, backlog = 0;
...
function write_data(worksheet, data, rowIndex, type) {
    /** For master file */
    header_list_1 = ["S.No.", "application_id", "category", "coap_registeration_number", "full_name", 
        "fathers_name", "date_of_birth", "email_id", "mobile_number", "branch_code", "gate_enrollment_number", 
        "gate_score", "Eligible for Admission (YES / No)", "Reasons, if not eligible"]

    /** For consolidated merit list */
    header_list_2 = ["S.No.", "application_id", "category", "coap_registeration_number", "full_name", "fathers_name", 
        "date_of_birth", "email_id", "mobile_number", "branch_code", "gate_enrollment_number", "gate_score", 
        "Merit Position", "Eligible for Admission (YES / No)", "Reasons, if not eligible"]

    /** For other merit lists */
    header_list_3 = ["S.No.", "application_id", "coap_registeration_number", "full_name", "fathers_name", 
        "date_of_birth", "email_id", "mobile_number", "branch_code", "gate_enrollment_number", 
        "gate_score", "Merit Position", "REMARKS"]
        ...
        data.forEach(element => {
            ...
            for(var i = 0; i < header_list_1.length-2; i++) {
                if(i == 0) {
                    ...
                    worksheet.cell(rowIndex, columnIndex).number(+element[header_list_1[i]]);
                }
                else {
                    worksheet.cell(rowIndex, columnIndex).string(element[header_list_1[i]]);
                }
...
async function generate_merit_list(info) {
    ...
    gate_enrollment_number, gate_score, degrees FROM applications_" + info.cycle_id + " WHERE offering_id = $1;",
    [info.offering_id]);
    ...
    /** obc applications */
    let obc_applications = all_applications.filter(element => element['category'] === 'OBC');
    obc_applications.sort(sort_function);

    /** sc applications */
    let sc_applications = all_applications.filter(element => element['category'] === 'SC');
    sc_applications.sort(sort_function);

    /** st applications */
    let st_applications = all_applications.filter(element => element['category'] === 'ST');
    st_applications.sort(sort_function);

    /** ews applications */
    let ews_applications = all_applications.filter(element => element['category'] === 'EWS');
    ews_applications.sort(sort_function);

    /** pwd applications */
    let pwd_applications = all_applications.filter(element => element['category'] === 'PWD');
