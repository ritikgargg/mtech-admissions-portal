...
async function generate_report(invalid_emails, wrong_status, under_review_list, count_not_selected, count_under_review, count_selected) {
...
const upload_results = async (req, res) => {
    ...
    */
    ...
    for(var i = 0; i < xlData.length; i++) {
        ...
        let status_check = (status.toLowerCase() !== 'selected') && (status.toLowerCase() !== 'not selected') && (status.toLowerCase() !== 'under review');

        if(status_check === true) {
            const result = await pool.query("SELECT * from applications_" + cycle_id + " WHERE email_id = $1 AND offering_id = $2;", [email, offering_id]);
            ...
            var db_status = ((status.toLowerCase() === 'selected') ? 2 : (status.toLowerCase() === 'under review' ? 1 : 0));
    ...
    /** Generate report */
    let report_workbook = await generate_report(invalid_emails, wrong_status, under_review_list, count_not_selected, count_under_review, count_selected);
