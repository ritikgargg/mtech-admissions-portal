...
function ChevronDots({
  ...
  let progressClass = `absolute top-1/2 left-0 h-1 transform -translate-y-1/2 bg-blue-400 transition-width ease-in-out duration-500`;
  const Steps = steps.map((step, index) => {
    let stepClass = "inline-block transform -translate-x-1/2 text-sm";
    if (currentStep - 1 > index) stepClass += " font-medium text-blue-400";
  ...
  return (
    <div className={finalClass} {...newProps}>
      <div className="h-2px w-full bg-gray-300 relative">
        ...
        >
          <div className="w-3 h-3 bg-blue-800 rounded-full absolute right-0 top-1/2 transform translate-x-1/2 -translate-y-1/2" />
        </div>
      </div>
